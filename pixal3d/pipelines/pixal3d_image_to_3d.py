...
class Pixal3DImageTo3DPipeline(Pipeline):
    ...
    @classmethod
    def from_pretrained(cls, path: str, config_file: str = "pipeline.json", load_rembg: bool = True) -> "Pixal3DImageTo3DPipeline":
        """
    ...
    def preprocess_image(self, input: Image.Image, bg_color: tuple = (0, 0, 0)) -> Image.Image:
        ...
        if has_alpha:
            ...
            if self.rembg_model is None:
                raise ValueError("Background removal disabled; provide a foreground RGBA image.")
            if self.low_vram:
        ...
        if len(bbox) < 16:
            raise ValueError("Empty or too small foreground mask")
